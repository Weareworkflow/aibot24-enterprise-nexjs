...
export async function GET(
  ...
  try {
    ...
    // Eliminamos el campo obsoleto del objeto agent para el panel
    const { systemPromptRegistered: _, ...safeAgent } = agent;

    return NextResponse.json({
      ...safeAgent,
      _id: undefined, // Ensure _id is still undefined if it was before
      compiledPrompt,
    });
...
export async function PUT(
  ...
  try {
    ...
    // Si se actualiza el prompt principal, sincronizamos el de seguimiento internamente
    // para que el agente de chat (Python) siga funcionando con un solo prompt real.
    if (updates.systemPrompt !== undefined) {
      updates.systemPromptRegistered = updates.systemPrompt;
    }
