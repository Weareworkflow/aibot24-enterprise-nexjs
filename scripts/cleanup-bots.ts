...
async function refreshBitrixToken() {
    if (!currentRefreshToken || !clientId || !clientSecret) {
        console.log("⚠️ No hay Refresh Token o credenciales configuradas para renovar.");
        return;
    }
...
async function callBitrix(method: string, params: any = {}) {
    ...
    // Si el token expiró (expired_token), intentamos renovar una vez
...
async function cleanup() {
    console.log(`🚀 Iniciando limpieza de bots en ${domain}...`);
    ...
    // 2. Eliminar cada bot
    // Nota: imbot.bot.list devuelve un objeto donde las llaves son los IDs de los bots
...
cleanup().catch(err => console.error("💥 Error crítico:", err));
